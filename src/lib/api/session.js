...
export default function session(headers){
  ...
  const passwordReset = (email) => client.post("/auth/password", {email, redirect_url: `${process.env.REACT_APP_HOST_URL}/sign_up`});

  return {signIn, signUp, updateUser, signOut, deleteUser, validate, passwordReset}
}
